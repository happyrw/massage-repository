...
const BookingForm = ({ category, price, session }: BookingProps) => {
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-5">
                ...
                <Button type="submit" className="w-full hover:text-black dark:hover:text-[#fff] hover:bg-transparent font-bold text-[#fff] transition-all duration-150 ease-linear dark:bg-black">{loading ? <LoaderIcon className="w-4 h-4 animate-spin" /> : "Accept"}</Button>
            </form>
        </Form>
    )
}
